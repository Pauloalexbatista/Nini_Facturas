...
- **Layout dos Itens (a partir da linha 3)**:
  1. `CODIGO`
  2. `Peso_KG` (2 casas decimais)
  3. `Lote`
  4. `Info_Tecnica` (Com quebra de linha/Wrap)
  5. `Preco` (2 casas decimais)
  6. `Zona_Pesca` (Consolidado de Zona Mar)
  7. `Metodo_Pesca` (Consolidado de Arte/Método Produção)
  8. `Origem` (Consolidado de Aquacultura/Massa Água)
  9. `Estado` (Estado)
  10. Outros campos de suporte (Abreviatura, Arquivo Original, etc.)
